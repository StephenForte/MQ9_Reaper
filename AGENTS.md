...
- **Secrets:**
  ...
  - Never commit `.env`.
- **Persistence:** Client download/upload of target JSON remains. Operators can also **Save to server** (public `POST /api/targets`) onto the persistent disk; Review can list/load those files. MCP `create_target` writes the same store. Config defaults: repo `config/app-config.md` is the seed; on Render, Admin writes the persistent-disk copy (`CONFIG_PATH` / `/var/data/app-config.md`). First boot seeds the disk from the repo file if missing. Target files live under `TARGETS_PATH` (or `dirname(CONFIG_PATH)/targets`).
...
lib/overpass.js           Overpass (OpenStreetMap) POI query helper (server)
...
public/
  ...
  js/
    ...
    overpass-candidates.js Client Overpass fetch → spaced candidate dots
...
| Route | Returns |
|-------|---------|
| `GET /api/health` | `{ ok, mapsKeyConfigured, geocodingConfigured, adminConfigured, mcpConfigured, mcpOauthConfigured, configPersistent, targetsPersistent }` (+ optional `geocodingProbe` when `?probe=geocode`) |
...
| `GET /api/overpass?lat=&lng=&radiusMiles=` | `{ places: [{ lat, lng, name, osmType, osmId, kind }], queryRadiusMiles }` — OSM POIs inside radius (optional `limit`; radius capped at 25 mi) |
...
| `POST /mcp` | Streamable HTTP MCP (Bearer `MCP_API_KEY` or OAuth access token; 503 if unset). Tools: `list_targets`, `get_target`, `create_target`, `summarize_library`. Resources: `targets://library`, `targets://{id}`. Prompts: `inspect_target`, `compare_targets`, `draft_target_package` |
| OAuth AS | `/.well-known/oauth-authorization-server`, `/.well-known/oauth-protected-resource/mcp`, `/authorize`, `/token` when `MCP_OAUTH_*` + public URL configured |
...
12. Default annotation on Save Targets: OSM `name` when the candidate came from Overpass, else place/address name when reverse-geocode finds one, else `{Region} Target N`; confidence `1`; priority `medium`. Show a non-blocking notice if reverse geocode fails. File-level `title` and `category` are required on new exports.
...
15. Candidate source defaults to Overpass (`candidateSource: overpass`); operators can switch to random disk in Selection. Optional `OVERPASS_API_URL` overrides the public Overpass endpoint.

## Local checklist

```bash
cp .env.example .env   # set Maps + geocoding keys; optional ADMIN_* / MCP_API_KEY / CONFIG_PATH
npm install
npm test
