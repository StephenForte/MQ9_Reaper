...
- **Secrets:**
  ...
  - `MCP_API_KEY` — optional (≥16 chars); enables Streamable HTTP MCP at `/mcp` for ChatGPT/Claude/Cursor. Clients send `Authorization: Bearer <key>`. If unset or too short, `/mcp` returns 503.
...
- **MCP:** Colocated Streamable HTTP at `/mcp` (`lib/mcp/`). Read + create only (no Admin delete/patch). Reuses `targetsStore`. Bearer `MCP_API_KEY` required when enabled.
...
| Route | Returns |
|-------|---------|
| `GET /api/health` | `{ ok, mapsKeyConfigured, geocodingConfigured, adminConfigured, mcpConfigured, configPersistent, targetsPersistent }` (+ optional `geocodingProbe` when `?probe=geocode`) |
...
| `POST /mcp` | Streamable HTTP MCP (Bearer `MCP_API_KEY`; 503 if unset). Tools: `list_targets`, `get_target`, `create_target`, `summarize_library`. Resources: `targets://library`, `targets://{id}`. Prompts: `inspect_target`, `compare_targets`, `draft_target_package` |
