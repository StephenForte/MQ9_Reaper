...
import { INVALID_STORED_TARGET_MESSAGE } from '../public/js/review-logic.js';
import {
  gameMetaFields,
...
export { INVALID_STORED_TARGET_MESSAGE };
...
/**
 * Best-effort list metadata when full schema validation fails.
 * @param {string} filePath
 * @param {string} filename
 * @returns {{ title: string, category: string, createdAt: string }}
 */
function peekListMeta(filePath, filename) {
  try {
    const raw = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
      const meta = normalizeTargetFileMeta(
      ...
      const createdAt =
        typeof raw.createdAt === 'string' && raw.createdAt.trim()
          ? raw.createdAt.trim()
          : '';
      return { title: meta.title, category: meta.category, createdAt };
    }
  } catch {
    // Corrupt / unreadable — fall through to filename defaults.
  }
  return { title: filename, category: '', createdAt: '' };
}
...
export function createTargetsStore(dirPath) {
  ...
  function readRaw(id) {
    ...
    try {
      ...
      return { ok: false, error: INVALID_STORED_TARGET_MESSAGE, status: 500 };
    }
    const validated = validateTargetFile(raw);
    if (!validated.ok) {
      return {
        ok: false,
        error: INVALID_STORED_TARGET_MESSAGE,
        status: 500,
      };
    }
  ...
  return {
    ...
    list() {
      ...
      for (const name of names) {
        ...
        // Still list corrupt / schema-invalid files so Admin can delete them.
        const filePath = filePathForId(dirPath, id);
        const partial = peekListMeta(filePath, name);
        items.push({
          id,
          title: partial.title,
    ...
    deleteMany(ids) {
      ...
      // Validate every id before unlinking any file so a late 400 cannot
      // leave earlier deletes applied while the API reports failure.
      for (const id of ids) {
        if (!isValidTargetId(id)) {
          return { ok: false, error: 'Invalid target id.', status: 400 };
        }
